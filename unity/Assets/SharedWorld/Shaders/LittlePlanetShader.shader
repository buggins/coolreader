...
//         }  
          ...
          float3 sphere2cube (float3 s, out float2 uv)
          {
            ...
            {
                r = float3 (sign (s.x), atan2 (s.y, abs (s.x)) / (PI / 4.0), atan2 (s.z, abs (s.x)) / (PI / 4.0));
                uv = 0.5 * r.yz + float2 (0.5, 0.5);
            }
            else 
            if ((abs (s.y) > abs (s.x)) && ((abs (s.y) > abs (s.z))))
            {
                r = float3 (atan2 (s.x, abs (s.y)) / (PI / 4.0), sign (s.y), atan2 (s.z, abs (s.y)) / (PI / 4.0));
                uv = 0.5 * r.zx + float2 (0.5, 0.5);
            }
            else
            {
              r = float3 (atan2 (s.x, abs (s.z)) / (PI / 4.0), atan2 (s.y, abs (s.z)) / (PI / 4.0), sign (s.z));
...
//            SplatmapMix(IN, defaultSmoothness, splat_control, weight, mixedDiffuse, o.Normal);
    ...
    float2 uvSplat1 = TRANSFORM_TEX(IN.tc.xy, _Splat1) * 100;
    float2 uvSplat2 = TRANSFORM_TEX(IN.tc.xy, _Splat2) * 100;
    float2 uvSplat3 = TRANSFORM_TEX(IN.tc.xy, _Splat3) * 100;
